...
- `/stats`에서 조회 월과 품목별·사이즈별·품목/사이즈별 순위를 선택한다. 출고 수량 내림차순이며 같은 수량은 공동 순위다. 반품은 차감하지 않는다.
- 제품 등록/수정에서 신발과 용품 모두 브랜드를 선택하거나 새 브랜드를 추가할 수 있다. 입고·출고·반품은 브랜드 → 제품 → 사이즈 순으로 선택한다.
- 기존 `엘에이기어` 브랜드는 API에서 `LA기어`로 표시하며 DB의 브랜드 ID와 제품 연결은 유지한다. 브랜드가 없는 제품은 `브랜드 미지정`에서 선택한다.
- 이번 변경은 새 DB 컬럼이나 데이터 재입력을 요구하지 않는다. 프론트엔드와 백엔드를 함께 배포한다. 이전 DB의 `base_price`/`memo` 컬럼은 기존 서버 시작 로직이 없을 때만 추가한다.
- 호환성 테스트: `cd backend` 후 `python -m unittest test_inventory_compatibility` (별도 메모리 DB 사용).
